...
namespace Bloggy.Tests
{
    public class SliceFixture
    {
        ...
        public static async Task ExecuteScopeAsync(Func<IServiceProvider, Task> action)
        {
            ...
            {
                var dbContext = scope.ServiceProvider.GetService<BloggyContext>();
        ...
        public static Task ExecuteDbContextAsync(Func<BloggyContext, Task> action) 
            => ExecuteScopeAsync(sp => action(sp.GetService<BloggyContext>()));

        public static Task ExecuteDbContextAsync(Func<BloggyContext, IMediator, Task> action) 
            => ExecuteScopeAsync(sp => action(sp.GetService<BloggyContext>(), sp.GetService<IMediator>()));

        public static Task<T> ExecuteDbContextAsync<T>(Func<BloggyContext, Task<T>> action) 
            => ExecuteScopeAsync(sp => action(sp.GetService<BloggyContext>()));

        public static Task<T> ExecuteDbContextAsync<T>(Func<BloggyContext, IMediator, Task<T>> action) 
            => ExecuteScopeAsync(sp => action(sp.GetService<BloggyContext>(), sp.GetService<IMediator>()));
